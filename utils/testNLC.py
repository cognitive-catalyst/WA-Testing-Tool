...
""" Test NLC instance using utterance
"""
import os
import csv
import asyncio
...
from __init__ import UTF_8, CONFIDENCE_COLUMN, \
    UTTERANCE_COLUMN, PREDICTED_INTENT_COLUMN, GOLDEN_INTENT_COLUMN, \
    save_dataframe_as_csv, INTENT_JUDGE_COLUMN, \
    TEST_OUT_FILENAME, BOOL_MAP, BASE_URL, \
    SCORE_COLUMN
...
async def fill_df(utterance, row_idx, out_df, classifier_id, apikey,
                  iam_apikey, sem, baseUrl):
    """ Send utterance to NLC and save response to dataframe
    """
    async with aiohttp.ClientSession(
            auth=aiohttp.BasicAuth(apikey, iam_apikey)) as session:
        MSG_ENDPOINT = baseUrl + '/v1/classifiers/{}/classify'
        url = MSG_ENDPOINT.format(classifier_id)

        # Replace newline chars before sending to NLC
        utterance = utterance.replace('\n', ' ')
        resp = await post(session, {'text': utterance}, url, sem)
        try:
...
def func(args):
    ...
    classifier_id = args.workspace_id
    ...
    tasks = (fill_df(out_df.loc[row_idx, test_column],
                     row_idx, out_df, classifier_id,
                     "apikey", args.iam_apikey, sem, args.url)
