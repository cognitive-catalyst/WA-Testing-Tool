...
def main():
    ...
    parser = ArgumentParser(
        description="Report all context statements used in action steps",
        epilog="Examples:\n"
               "  python -m cli.context_usage                    # All actions\n"
               "  python -m cli.context_usage action_1           # Single action\n"
               "  python -m cli.context_usage action_1 action_2  # Multiple actions",
    ...
    context_usage_df = action_analyzer.context_usage(*action_ids, return_as=OutputFormat.DATAFRAME)

    context_usage_df = context_usage_df.sort_values(["action_id", "step_number"])

    default_file_name = "all_context_usage.csv"
    if len(action_ids):
        default_file_name = f"context_usage ({', '.join(action_ids)}).csv"

    create_directory(args.output_path)
    output_path = get_output_save_path(args.output_path, default_file_name)
    context_usage_df.to_csv(output_path, index=False)
    
    print(f"Context usage saved to: {output_path}")
