...
def main():
    ...
    parser = ArgumentParser(
        description="Extract validation settings (input validation rules) for action steps",
        epilog="Examples:\n"
               "  python -m cli.validation_settings                    # All actions\n"
               "  python -m cli.validation_settings action_1           # Single action\n"
               "  python -m cli.validation_settings action_1 action_2  # Multiple actions",
    ...
    validation_settings_df = action_analyzer.validation_settings(*action_ids, return_as=OutputFormat.DATAFRAME)

    validation_settings_df = validation_settings_df.sort_values(["action_id", "step_number"])

    default_file_name = "all_validation_settings.csv"
    if len(action_ids):
        default_file_name = f"validation_settings ({', '.join(action_ids)}).csv"

    create_directory(args.output_path)
    output_path = get_output_save_path(args.output_path, default_file_name)
    validation_settings_df.to_csv(output_path, index=False)
    
    print(f"Validation settings saved to: {output_path}")
