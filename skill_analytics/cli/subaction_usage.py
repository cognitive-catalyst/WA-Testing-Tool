...
def main():
    ...
    parser.add_argument('subactions', nargs='*', help='List of subactions names to search for.')
    ...
    subactions = clean_cli_list(args.subactions)
    if len(subactions):
        subaction_usage_df = analyzer.search_for_subactions(*subactions, return_as="csv")
        default_file_name = f"{', '.join(subactions)} usage.csv"
    else:
        subaction_usage_df = analyzer.get_all_subaction_usage(return_as="csv")
        default_file_name = "all subaction usage.csv"
