...
def main():
    ...
    parser = ArgumentParser(
        description="Find all custom extension calls in the assistant",
        epilog="Example: python -m cli.extension_usage -i assistant.json -o ./reports"
    )
    parser.add_argument(
    ...
    extension_usage_df = resolver_analyzer.extension_usage(return_as=OutputFormat.DATAFRAME)

    extension_usage_df = extension_usage_df.sort_values(["action_id", "step_number"])

    default_file_name = "extension_usage.csv"
    ...
    print(f"Extension usage saved to: {output_path}")
